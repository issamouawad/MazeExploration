...
+!moveavail:me(M) & finished(M) <-.print('finished wont move').
//stop handling deadends if an agent is finished, and we are following the last_dir, since all agents will do the same and deadend information is not needed
...
//if both options are taken, take the one that is not deadend
...
+!moveavail:last_dir(A) & avail(A) & .count(avail(_))=2 & avail(B) & not A=B & me(M) & pos(X,Y)&choice(X,Y,A,C1) &  choice(X,Y,B,C2) & not deadend(X,Y,B) <-
			!addChoice(X,Y,M,B); -+last_dir(B); do(B).
//if the current direction is no longer available, take another suitable choice
...
// if a deadend was detected, and in the current junction, all legs but one are now declared deadends, propagate the deadend to the parent junction
+!addDeadendfound(X,Y,Dir): .count(deadend(X,Y,_)) = .count(avail(_)) -2<-
					+deadend(X,Y,Dir);.print('propagate dead end');-+deadendfound .
//otherwise, just mark the leg dead, and delete the deadendfound signal
+!addDeadendfound(X,Y,Dir):true<-+deadend(X,Y,Dir); -deadendfound.

//adding choice beliefe, to the leg chosen , and to the leg from which the agent came
